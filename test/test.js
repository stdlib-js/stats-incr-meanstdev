...
tape( 'the function returns an accumulator function', function test( t ) {
	t.strictEqual( typeof incrmeanstdev(), 'function', 'returns expected value' );
	t.end();
});

tape( 'the function returns an accumulator function (output)', function test( t ) {
	t.strictEqual( typeof incrmeanstdev( [ 0.0, 0.0 ] ), 'function', 'returns expected value' );
	t.end();
});
...
tape( 'the sample standard deviation is `0` until at least 2 datums have been provided', function test( t ) {
	...
	ms = acc();
	t.strictEqual( ms, null, 'returns expected value' );

	ms = acc( 3.0 );
	t.strictEqual( ms[ 1 ], 0.0, 'returns expected value' );
